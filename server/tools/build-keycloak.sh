...
if [ "$GIT_REPO" != "" ]; then
    ...
    cd /opt/jboss/
    curl --insecure -L $KEYCLOAK_DIST | tar zx
    mv /opt/jboss/keycloak-?.?.?* /opt/jboss/keycloak
fi
...
curl  --insecure -O https://central.maven.org/maven2/mysql/mysql-connector-java/$JDBC_MYSQL_VERSION/mysql-connector-java-$JDBC_MYSQL_VERSION.jar
...
curl --insecure -L https://central.maven.org/maven2/org/postgresql/postgresql/$JDBC_POSTGRES_VERSION/postgresql-$JDBC_POSTGRES_VERSION.jar > postgres-jdbc.jar
...
curl --insecure -L https://central.maven.org/maven2/org/mariadb/jdbc/mariadb-java-client/$JDBC_MARIADB_VERSION/mariadb-java-client-$JDBC_MARIADB_VERSION.jar > mariadb-jdbc.jar
